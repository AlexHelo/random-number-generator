  ...
  function mclmCalculate(){
    ...
    for (let i = 0; i < valueHolder[0].length; i++) {
      ...
      for (let j = 0; j < inputList.length; j++) {
        ...
        if (j === 0){
          wHold = inputList[j].Multiplicador * valueHolder[j][i] % inputList[j].Modulo
        }
        else {

          wHold = wHold - inputList[j].Multiplicador * valueHolder[j][i] % inputList[j].Modulo
        }

      }   
      values.push(modulo(wHold, mod))
    }
